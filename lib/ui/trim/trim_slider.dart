...
class _TrimSliderState extends State<TrimSlider> with AutomaticKeepAliveClientMixin<TrimSlider> {
  ...
  late final double _horizontalMargin = widget.horizontalMargin + widget.style.edgeWidth;
  ...
  late final _edgesTouchMargin = max(widget.style.edgeWidth, _touchMargin);
  // Position line touch margin come from it size, but minimum is [margin]
  late final _positionTouchMargin = max(widget.style.positionLineWidth, _touchMargin);
  ...
  void _updateTrim() {
    if (widget.controller.minTrim != _getRectToTrim(_rect.left) || widget.controller.maxTrim != _getRectToTrim(_rect.right)) {
      ...
      if (_isExtendTrim) {
        setState(() {
          _rect = Rect.fromLTWH(
              _horizontalMargin, _rect.top, _geTrimToRect(widget.controller.maxTrim) - _geTrimToRect(widget.controller.minTrim), _rect.height);
        });
